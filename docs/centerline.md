...
The method behind `analysis/centerline.py`: how the skeleton is built,
how branches are numbered, what the branching ratios mean, and what has to
hold before they can be believed. Run everything below from the repository
root.

See the [README](../README.md) for the rest of the pipeline.
...
```bash
python -m analysis.centerline --input av_seg.nii.gz --label 4 --compare-label 3 \
...
```bash
python -m analysis.centerline --input av.nii.gz --label 1 --compare-label 2 --quality-csv qual_A.csv ...
python -m analysis.centerline --input av.nii.gz --label 2 --compare-label 1 --quality-csv qual_V.csv ...
```
...
```bash
python -m analysis.centerline --input artery.nii.gz --ordering strahler_dd \
...
```bash
python -m analysis.phantom --spacing 1.25 0.799 1.25 --orders 7 --output tree.nii.gz
```
...
```bash
python -m analysis.calibrate --from-csv reference/calibration.csv --measured-rd 1.432
```
...
`reference/calibration.csv` in this repository is one such sweep, on a
...
```bash
python -m analysis.radius_audit --spacing 1.188,0.799,1.313 --orders 7 \
